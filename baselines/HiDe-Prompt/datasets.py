...
def split_custom_dataset(datasets, n_tasks=19):
    ...
    all_classes_train = list(class_to_indices_train.keys())[:n_tasks*n_classes]
    all_classes_val = list(class_to_indices_val.keys())[:n_tasks*n_classes]
    all_classes_train.sort()
    all_classes_val.sort()
    tasks_train = [[] for _ in range(n_tasks)]
    tasks_val = [[] for _ in range(n_tasks)]
    ...
    cls_task_datasets = [dict() for i in range(len(tasks_train))]
    for task_id, class_ids in enumerate(tasks_train):
        task_indices = []
        for cls in class_ids:
            task_indices.extend(class_to_indices_train[cls])
            class_dataset = Subset(full_dataset_train, class_to_indices_train[cls])
            cls_task_datasets[task_id][cls] = dict()
            cls_task_datasets[task_id][cls]['train'] = class_dataset
    ...
    for task_id, class_ids in enumerate(tasks_val):
        ...
        for cls in class_ids:
            ...
            class_dataset = Subset(full_dataset_val, class_to_indices_val[cls])
            cls_task_datasets[task_id][cls]['val'] = class_dataset

        task_dataset = Subset(full_dataset_val, task_indices)
        task_datasets[task_id].extend([task_dataset])
    
    return task_datasets, cls_task_datasets

def build_continual_dataloader_vtab_19t(args, n_tasks=19, data_path=""):
    dataloader = list()
    dataloader_per_cls = dict()
    class_mask = list()
    target_task_map = dict()
    ...
    for i in range(len(dataset_list)):
        dataset_train, dataset_val = get_dataset(dataset_list[i], transform_train, transform_val, args)
        datasets += [{'data_id':i ,'data_train':dataset_train, 'data_test': dataset_val}]
        splited_dataset, splitted_class_dataset = split_custom_dataset(datasets, n_tasks)
    ...
    for i in range(n_tasks):
        ...
        if class_mask is not None:
            ...
            for j in class_mask[i]:
                target_task_map[j] = i
            nb_classes += 10

        splited_dataset_per_cls.update(splitted_class_dataset[i])
...
def build_continual_dataloader(args):
    ...
    if args.dataset.startswith('Split-'):
        ...
        if args.dataset == '5-datasets':
            dataset_list = ['SVHN', 'MNIST', 'CIFAR10', 'NotMNIST', 'FashionMNIST']
        elif args.dataset == 'VTAB5T-large':
            dataset_list = ['patch_camelyon', 'eurosat', 'oxford_flowers102', 'oxford_iiit_pet', 'resisc45']
            # dataset_list = ['patch_camelyon', 'resisc45']
        else:
            dataset_list = args.dataset.split(',')
...
def get_dataset(dataset, transform_train, transform_val, args, target_transform=None):
    if dataset == 'CIFAR100':
        ...
        dataset_train = vtab(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = vtab(args.data_path, train=False, download=False, transform=transform_val).data
    
    # VTAB 5T large
    elif dataset == 'eurosat':
        dataset_train = EuroSAT(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = EuroSAT(args.data_path, train=False, download=False, transform=transform_val).data

    elif dataset == 'oxford_flowers102':
        dataset_train = Flowers(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = Flowers(args.data_path, train=False, download=False, transform=transform_val).data

    elif dataset == 'oxford_iiit_pet':
        dataset_train = Pets(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = Pets(args.data_path, train=False, download=False, transform=transform_val).data

    elif dataset == 'patch_camelyon':
        dataset_train = PatchCamelyon(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = PatchCamelyon(args.data_path, train=False, download=False, transform=transform_val).data

    elif dataset == 'resisc45':
        dataset_train = Resisc(args.data_path, train=True, download=False, transform=transform_train).data
        dataset_val = Resisc(args.data_path, train=False, download=False, transform=transform_val).data
