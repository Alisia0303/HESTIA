...
def train_one_batch(model: torch.nn.Module, original_model: torch.nn.Module,
                    criterion, input: torch.Tensor, target: torch.Tensor, optimizer: torch.optim.Optimizer,
    ...
    for epoch in range(args.epochs):
        ...
        output = model(input, cls_features=cls_features, train=set_training_mode)
        logits = output['logits']
        
        # CANNOT MASK BECAUSE WE DONT KNOW TASK-ID.
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        for idx, (input, target) in enumerate(data_loader[task_id]['train']):
            ...
            train_stats = train_one_batch(model=model, original_model=original_model, criterion=criterion,
                                          input=input, target=target, optimizer=optimizer,
                                          device=device, max_norm=args.clip_grad,
                                          set_training_mode=True, task_id=task_id, class_mask=new_classes, args=args, )
        ...
        #print(f"Total inference time for task {task_id} runs: {total_time:.4f} ms")
