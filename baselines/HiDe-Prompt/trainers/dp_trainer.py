...
from datasets import build_continual_dataloader, build_continual_dataloader_vtab_19t
...
def train(args):
    ...
    if args.dataset == 'VTAB19T':
        data_loader, data_loader_per_cls, class_mask, target_task_map = build_continual_dataloader_vtab_19t(args,
                                                                n_tasks=args.num_tasks, data_path=args.data_path)
    else:
        data_loader, data_loader_per_cls, class_mask, target_task_map = build_continual_dataloader(args)

    # data_loader = torch.load(args.data_path + "dataloader_output.pt")
    # class_mask = np.load(args.data_path + "class_mask.npy", allow_pickle=True).tolist()
    # data_loader_per_cls = torch.load(args.data_path + "dataloader_per_cls_output.pt")
    
    # target_task_map = dict()
    # for i in range(len(class_mask)):
    #     for j in range(len(class_mask[i])):
    #         target_task_map[class_mask[i][j]] = i
    # print("class_mask ", class_mask)
    # print("nb classes ", args.nb_classes)
    # print("target task map ", target_task_map)
