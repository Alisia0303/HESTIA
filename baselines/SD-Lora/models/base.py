...
#         self.topk = 5
#         self.best_task_acc = {}  # task_id -> best accuracy so far
#         self.evaluation_control = []
...
#     def _task_free_update(self, train_loader, test_loader, optimizer, scheduler):
#         pass
...
#         num_tasks = 20
#         total_accuracy = 0
#         # self.truth_real_tasks + 1
#         for i in range(num_tasks):
#             print(f"EVALUATE_CURRENT_MODEL ON TASK {i}")
#             test_truth_task_dataset = self.task_test_datasets[i]
...
#                 num_workers=8,
#             )
#             y_pred, y_true = self._eval_cnn(task_test_loader)
#             accy = self._evaluate(y_pred, y_true)
#             acc = accy['top1']
#             total_accuracy = total_accuracy + accy['top1']

#             # ---------- Forgetting ----------
#             if i not in self.best_task_acc:
#                 self.best_task_acc[i] = acc
#                 forgetting = 0.0
#             else:
#                 forgetting = self.best_task_acc[i] - acc
#                 self.best_task_acc[i] = max(self.best_task_acc[i], acc)

#             total_forgetting += forgetting
#             print(
#                 f"Task {i} | Acc={acc:.2f}% | "
#                 f"Best={self.best_task_acc[i]:.2f}% | "
#                 f"Forgetting={forgetting:+.2f}%"
#             )    


        
#         avg_acc = total_accuracy / num_tasks
#         avg_forgetting = total_forgetting / num_tasks
#         print(f"Average Accuracy over {num_tasks} tasks: {avg_acc:.2f}%")
#         print(f"Average Forgetting over {num_tasks} tasks: {avg_forgetting:.2f}%")

#         # ---------- Global CNN ----------
#         y_pred, y_true = self._eval_cnn(self.global_test_loader)
...
#         self._network.eval() 
#         y_pred, y_true = [], []
#         for _, (inputs, targets, task) in enumerate(loader):
#             inputs = inputs.to(self._device)
