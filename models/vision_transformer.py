...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,):
        ...
        # IA3: Learned vectors of size 'dim' initialized to 1s
        self.l_k = nn.Parameter(torch.ones(dim))
        self.l_v = nn.Parameter(torch.ones(dim))
    ...
    def forward(self, x, *args):
        ...
        # 1. Split q, k, v BEFORE reshaping to heads to apply IA3 easily
        # qkv is (B, N, 3*C). We split into 3 tensors of (B, N, C)
        q, k, v = qkv.chunk(3, dim=-1)

        # --- IA3 INJECTION ---
        # Apply scaling to the full dim vectors
        k = k * self.l_k
        v = v * self.l_v
        # ---------------------

        # 2. Now reshape for multi-head attention
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.l_ff = nn.Parameter(torch.ones(int(dim * mlp_ratio)))
    ...
    def forward(self, x, prompt=None):
        x1 = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), prompt)))
        x1_norm2 = self.norm2(x1)
        x1_mlp_act = self.mlp.act(self.mlp.fc1(x1_norm2)) * self.l_ff
        x1_mlp = self.mlp.drop2(self.mlp.fc2(self.mlp.drop1(x1_mlp_act)))
        x_final = x1 + self.drop_path2(self.ls2(x1_mlp))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            head_type='token',  distilled=False,):
        """
        Args:
    ...
    def forward_features(self, x_embed, task_id=-1, train=False):
        B = x_embed.shape[0]
